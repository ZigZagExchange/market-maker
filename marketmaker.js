...
if (infureKey) {
  ethersProvider = new ethers.providers.InfuraProvider("mainnet", infureKey);
} else {
  ...
  switch (msg.op) {
    ...
    case "orders":
      ...
      orders.forEach((order) => {
        ...
        } else if (["sending order already", "badprice"].includes(fillable.reason)) {
...
function genQuote(chainId, marketId, side, baseQuantity) {
  ...
  const primaryPrice = mmConfig.invert ? 1 / PRICE_FEEDS[mmConfig.priceFeedPrimary] : PRICE_FEEDS[mmConfig.priceFeedPrimary];
...
async function sendFillRequest(orderreceipt, accountId) {
  ...
  const sellQuantityParsed = syncProvider.tokenSet.parseToken(tokenSell, sellQuantity);
  ...
  rememberOrder(chainId, marketId, orderId, quote.quotePrice, sellSymbol, sellQuantity, buySymbol, buyQuantity);
}
...
async function broadcastFill(chainId, orderId, swapOffer, fillOrder, wallet) {
  ...
  if (nonce <= userNonce) {
    const orderCommitMsg = {
      op: "orderstatusupdate",
      args: [[[chainId, orderId, "r", null, "Order failed userNonce check."]]],
    };
    sendMessage(JSON.stringify(orderCommitMsg));
    return;
  }
  ...
  if (FEE_TOKEN) {
    ...
    feeToken = FEE_TOKEN_LIST.includes(swapOffer.tokenSell) ? swapOffer.tokenSell : "ETH";
  }
...
async function uniswapV3Setup(uniswapV3Address) {
  const results = uniswapV3Address.map(async (address) => {
    try {
      ...
      let [slot0, addressToken0, addressToken1] = await Promise.all([provider.slot0(), provider.token0(), provider.token1()]);
      ...
      let [decimals0, decimals1] = await Promise.all([tokenProvier0.decimals(), tokenProvier1.decimals()]);
...
function indicateLiquidity(pairs = MM_CONFIG.pairs) {
  for (const marketId in pairs) {
    ...
    const midPrice = mmConfig.invert ? 1 / PRICE_FEEDS[mmConfig.priceFeedPrimary] : PRICE_FEEDS[mmConfig.priceFeedPrimary];
...
async function afterFill(chainId, orderId, wallet) {
  ...
  const buyTokenParsed = syncProvider.tokenSet.parseToken(order.buySymbol, order.buyQuantity);
  const sellTokenParsed = syncProvider.tokenSet.parseToken(order.sellSymbol, order.sellQuantity);
  ...
  if (mmConfig.delayAfterFill) {
    ...
    if (!Array.isArray(mmConfig.delayAfterFill) || !mmConfig.delayAfterFill[1]) {
  ...
  const increaseSpreadAfterFillMinSize = mmConfig.increaseSpreadAfterFill?.[2] ? mmConfig.increaseSpreadAfterFill[2] : 0;
  if (mmConfig.increaseSpreadAfterFill && order.baseQuantity > increaseSpreadAfterFillMinSize) {
  ...
  const changeSizeAfterFillMinSize = mmConfig.changeSizeAfterFill?.[2] ? mmConfig.changeSizeAfterFill[2] : 0;
  if (mmConfig.changeSizeAfterFill && order.baseQuantity > changeSizeAfterFillMinSize) {
